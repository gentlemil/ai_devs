...
if (major < MIN_NODE_VERSION) {
  console.error(
    `\x1b[31mError: Node.js ${MIN_NODE_VERSION}+ is required\x1b[0m`,
  );
...
if (requestedProvider && !VALID_PROVIDERS.has(requestedProvider)) {
  console.error(
    "\x1b[31mError: AI_PROVIDER must be one of: openai, openrouter\x1b[0m",
  );
  process.exit(1);
}
...
const resolveProvider = () => {
  if (requestedProvider) {
    if (requestedProvider === "openai" && !hasOpenAIKey) {
      console.error(
        "\x1b[31mError: AI_PROVIDER=openai requires OPENAI_API_KEY\x1b[0m",
      );
      process.exit(1);
    }
    ...
    if (requestedProvider === "openrouter" && !hasOpenRouterKey) {
      console.error(
        "\x1b[31mError: AI_PROVIDER=openrouter requires OPENROUTER_API_KEY\x1b[0m",
      );
      process.exit(1);
    }
